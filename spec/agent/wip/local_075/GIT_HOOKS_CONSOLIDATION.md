...
| ファイル | 変更種別 | 変更内容 |
|----------|----------|----------|
| `.lintstagedrc.json` | 削除 | `.lintstagedrc.mjs` に移行 |
| `.lintstagedrc.mjs` | 新規 | ESLint / clippy を追加、`*.rs` は関数形式でファイル引数を無視 |
...
lint-staged の Rust ファイル向け設定では、対象ファイルの有無をトリガーとして `cargo clippy -p wasm-pkg --all-targets -- -D warnings` を実行する。lint-staged はファイルパスを引数として渡すが、clippy はファイル単位の指定に対応しないため、渡されたファイルパスを無視してプロジェクト全体を対象にする。

## 4. 実装仕様

### 4.1 `.lintstagedrc.mjs` (`.lintstagedrc.json` から移行)

`cargo clippy` にファイル引数を渡すとエラーになることを確認済みのため、`*.rs` は関数形式でファイル引数を無視する構成とした。設定ファイルは `.lintstagedrc.json` から `.lintstagedrc.mjs` に変更。
...
`cargo fmt --` はファイル引数を受け取り指定ファイルのみフォーマットするが、関数形式ではファイル引数が渡されないため、`cargo fmt` がワークスペース全体をフォーマットする。Rust ファイルの変更頻度は低いため許容範囲。
...
### 5.3 `cargo clippy` のファイル引数の挙動確認 (実施済み)

ファイル引数を渡した場合の `cargo clippy` 挙動を確認した結果、エラーとなることを確認:
...
このため、`.lintstagedrc.mjs` で関数形式を採用し、ファイル引数を渡さない構成とした。

## 6. 実装チェックリスト

- [x] `.git/hooks/pre-commit` 削除
- [x] `.git/hooks/pre-commit.ps1` 削除
- [x] `.lintstagedrc.json` → `.lintstagedrc.mjs` に移行、ESLint + clippy 追加
- [x] `cargo clippy` のファイル引数挙動確認 → エラー発生を確認、関数形式に変更
- [x] `.husky/pre-push` 新規作成
- [x] pre-commit 動作確認: ESLint エラー中断 (lint-staged 実行で確認)
- [x] pre-commit 動作確認: 非対象ファイルのみの変更で不要なチェックが走らない (dry-run で確認)
- [x] pre-push 動作確認: `pnpm lint` + `tsc` + `format:check` 通過
- [ ] pre-push 動作確認: エラー時に push 中断 (実際の push 時に確認)
