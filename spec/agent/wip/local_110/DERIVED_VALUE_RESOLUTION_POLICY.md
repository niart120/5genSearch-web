...
| ファイル | 変更種別 | 変更内容 |
|----------|----------|----------|
| `src/stores/search/results.ts` | 調査・必要に応じて修正 | pending 値の consumer / target 分離を store 型として表現する |
| `src/lib/navigate.ts` | 調査・必要に応じて修正 | `navigateWithSeedOrigins` の target を pending store に保持する |
| `src/components/forms/seed-input-section.tsx` | 調査・必要に応じて修正 | `pendingSeedOrigins` / `pendingDetailOrigins` の consume-once を mount 時だけに限定しない |
| `src/features/datetime-search/components/datetime-search-page.tsx` | 調査・必要に応じて修正 | `pendingTargetSeeds` を mount 後の store 更新でも取り込む |
| `src/features/needle/components/needle-page.tsx` | 調査・必要に応じて修正 | `pendingDetailOrigins['needle']` を mount 後の store 更新でも取り込む |
| `src/features/pokemon-list/components/pokemon-params-form.tsx` | 調査・必要に応じて修正 | `genConfig` 由来の表示 state と外部値の同期方針を定める |
| `src/components/forms/egg-params-form.tsx` | 調査・必要に応じて修正 | 親 IV 表示 state と `genConfig` 表示 state の同期方針を定める |
| `src/features/pokemon-list/components/pokemon-filter-form.tsx` | 調査・必要に応じて修正 | トグル付き内部フィルタの外部変更追従条件を定める |
| `src/components/forms/egg-filter-form.tsx` | 調査・必要に応じて修正 | トグル付き内部フィルタの外部変更追従条件を定める |
...
useEffect(() => {
  ...
  setTargetSeedsRaw(pendingTargetSeeds.map((seed) => toHex(seed, 8)).join('\n'));
...
- [ ] `pendingTargetSeeds` の mount 後反映を実装する
- [ ] `pendingSeedOrigins` / `pendingDetailOrigins` の mount 後反映を実装する
- [ ] 数値入力・IV 入力の外部値同期を必要箇所へ適用する
- [ ] トグル付きフィルタの外部 reset 同期方針を実装へ反映する
- [ ] 上記の回帰テストを追加する
