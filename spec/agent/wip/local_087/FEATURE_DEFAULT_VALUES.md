...
- [ ] datetime-search: `DEFAULT_DATE_RANGE` を `2000/1/1 ~ 2099/12/31` に変更
- [ ] datetime-search: `useGpu` 初期値を `true` に変更
- [ ] datetime-search: `targetSeedsRaw` 初期値を BW 6V seeds に変更 (`getDefaultTargetSeeds` 関数追加)
- [ ] mtseed-search: `useGpu` 初期値を `true` に変更
- [ ] egg-search: `DEFAULT_DATE_RANGE` を `getTodayDateRange()` に変更
- [ ] egg-search: `DEFAULT_GEN_CONFIG.max_advance` を `30` に変更
- [ ] egg-list: `DEFAULT_GEN_CONFIG.max_advance` を `30` に変更
- [ ] pokemon-list: `seedInputMode` 初期値を `manual-startup` に変更
- [ ] pokemon-list: `DEFAULT_ENCOUNTER_PARAMS.genConfig.max_advance` を `30` に変更
- [ ] pokemon-list: `speciesOptions` ロード完了時の先頭エントリ自動選択ロジック追加
- [ ] needle: `maxAdvance` 初期値を `30` に変更
- [ ] tid-adjust: `DEFAULT_DATE_RANGE` を `getTodayDateRange()` に変更
- [ ] ユニットテスト追加 (`getTodayDateRange`, `getDefaultTargetSeeds`)
- [ ] コンポーネントテスト追加/更新 (各 feature の初期表示)
- [ ] 動作確認: 各 feature ページの初期表示・検索実行が正常に動作すること
