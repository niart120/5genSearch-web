...
生成手順は本書の仕様とし、Rust の型・関数名は実装に向けたインタフェース案として記載する。
...
- [x] 生成条件と消費順序を文書化する
- [x] Rust 内部の入出力インタフェース案を記載する
- [ ] 非公開フィールドを持つ生成条件型と構築時検証を実装する
- [ ] 検証済みの条件を受け取る一個体生成処理を実装する
- [ ] 生成結果・最終 LCG 状態の単体テストを実装して実行する
- [ ] Rust ディレクトリ構成と検証結果を更新する
