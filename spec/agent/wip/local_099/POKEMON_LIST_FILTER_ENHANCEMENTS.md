...
3. **エンカウント結果フィルタ** --- `EncounterResult` による結果種別条件 (`DustCloud` / `PokemonShadow` / `Fishing` 時のみ表示)
...
- 通常釣り (`Fishing`) では 50% の確率で釣り失敗 (`FishingFailed`) が発生し、結果一覧に混在する。成功分のみ表示したい需要がある
...
| エンカウント結果フィルタ | `DustCloud` / `PokemonShadow` でポケモンのみ・アイテムのみ、`Fishing` で釣り失敗除外が可能になる |
...
| エンカウント結果 | `encounterType` が `DustCloud` / `PokemonShadow` / `Fishing` のいずれか |
...
const ENCOUNTER_RESULT_ENCOUNTER_TYPES: Set<EncounterType> = new Set([
  'DustCloud', 'PokemonShadow', 'Fishing',
]);
```
