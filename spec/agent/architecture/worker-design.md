...
5. Worker は `done` または `error` を返して実行を終了
6. WorkerPool は終了した Worker に次の検索セッションのタスクを配布

### 11.2 注意点

- WASM 処理中は即時停止不可 (バッチ単位)
- キャンセル前に受信済みの途中結果は保持・表示
- タスク ID は検索セッションごとに一意とし、キャンセル済みセッションから遅れて届いた進捗・結果・完了通知は現行セッションへ反映しない
- 実行中の Worker に次の検索タスクを重ねて送らず、旧タスクの終了通知を受信してから再利用する
