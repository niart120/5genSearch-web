...
メインコンテンツにグローバルな `max-width` 制約は設けない。サイドバーは左端固定、メインコンテンツは `flex-1` で残り幅を全て使用する。個別コンポーネントが必要に応じて自身の `max-width` を設定する。

| 対象 | 方針 | 備考 |
|------|------|------|
| メインコンテンツ全体 | `max-width` なし | テーブル・フォームの表示領域を最大化 |
| テーブル | 制限なし | 横スクロール (`overflow-x-auto`) で対応 |
| フォーム | コンポーネント単位で制約 | 例: `max-w-2xl`, `max-w-prose` |
| テキスト段落 | コンポーネント単位で制約 | 例: `max-w-prose` |
...
| WQHD (2560x1440) | 100% | 2560px | メインコンテンツが全幅使用 |
| 4K (3840x2160) | 150% | 2560px | メインコンテンツが全幅使用 |
...
| 脅威 | 対策 |
|------|------|
| 超広幅でコンテンツが横に伸びすぎる | 個別コンポーネントで必要に応じて `max-width` を設定。テーブルは全幅活用 |
