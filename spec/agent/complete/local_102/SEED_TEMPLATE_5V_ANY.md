...
| 登録 Seed 数 | BW A抜け5V: 107 件, BW C抜け5V: 170 件, BW2 A抜け5V: 135 件, BW2 C抜け5V: 134 件 |
...
```typescript
{
  ...
  category: 'stationary',
  seeds: [
    // mt_offset_0_5V_anyA.csv より 107 件
...
},
{
  ...
  category: 'stationary',
  seeds: [
    // mt_offset_0_5V_anyC.csv より 170 件
...
```typescript
{
  ...
  category: 'stationary',
  seeds: [
    // mt_offset_2_5V_anyA.csv より 135 件
...
- [x] `src/data/seed-templates.ts` に BW A抜け5V テンプレートを追加
- [x] `src/data/seed-templates.ts` に BW C抜け5V テンプレートを追加
- [x] `src/data/seed-templates.ts` に BW2 A抜け5V テンプレートを追加
- [x] `src/data/seed-templates.ts` に BW2 C抜け5V テンプレートを追加
- [x] `pnpm exec tsc -b --noEmit` で型エラーがないことを確認
- [x] `pnpm test:run` で既存テスト通過を確認 (1393 passed)
