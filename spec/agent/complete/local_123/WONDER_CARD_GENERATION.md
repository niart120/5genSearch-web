...
| ファイル | 変更種別 | 変更内容 |
|----------|----------|----------|
| `wasm-pkg/src/generation/flows/wondercard.rs` | 新規 | 専用の入力型・結果型、一個体生成関数、単体テストと固定期待値 |
...
| 型とテスト | 条件型・構築時検証・生成本体を `wondercard.rs` にまとめ、単体テストも既存の生成フローに合わせて同ファイル内の `#[cfg(test)] mod tests` に配置 |

条件型は本生成処理だけがフィールドを読むため、同じモジュールに置いて非公開フィールドを維持する。型だけを別モジュールへ移すためのアクセサーや可視性拡大は行わない。`RawWonderCardData` も 3.3 の責務に合わせて専用型を維持する。
...
2026-09-10 に実行。既存関数の再利用後にも、Rust 全体テスト・Clippy・フォーマット検査を再実行して成功した。テストを同一ファイル内へ戻した後は、配達員の 16 件のテストとフォーマット検査を再実行した。
