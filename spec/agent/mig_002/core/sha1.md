...
message[12] に格納されるキー入力値。DS 起動時に押下されていたボタンを 12 ビットマスクで表現。
...
```rust
/// キー入力なし
const KEY_NONE: u32 = 0x2FFF;

/// A + B 同時押し
/// key_mask = 0b0000_0000_0011 = 0x0003
/// key_code = 0x0003 XOR 0x2FFF = 0x2FFC
const KEY_A_B: u32 = 0x2FFC;

/// L + R 同時押し
/// key_mask = 0b0011_0000_0000 = 0x0300
/// key_code = 0x0300 XOR 0x2FFF = 0x2CFF
const KEY_L_R: u32 = 0x2CFF;
```

#### 2.5.4 実装

```rust
/// キーマスクから key_code を生成
#[inline]
pub fn build_key_code(key_mask: u16) -> u32 {
    (key_mask as u32) ^ 0x2FFF
}

/// key_code からキーマスクを復元
#[inline]
pub fn key_code_to_mask(key_code: u32) -> u16 {
    (key_code ^ 0x2FFF) as u16
}
```
