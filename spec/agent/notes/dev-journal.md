...
当面の方針: `spec/agent/wip/local_118/POKEMON_SEARCH.md` の新検索では既存フォームの扱いを維持し、今回の必須改修に含めない。対応時は既存生成画面での再現を確認し、取得中の開始操作と古い取得結果の扱いを検討する。
...
当面の方針: `spec/agent/wip/local_119/POKEMON_SEARCH_ENGINE.md` の新検索は既存進捗基盤へ接続する。共通基盤の拡張は今回行わず、必要になった時点で既存 CPU / GPU 検索への影響を含めて別途設計する。
