...
## 2026-02-10: PC 版サイドバー配置の再検討

現状: PC 版はサイドバー (DS 設定) を左側、メインコンテンツを右側に配置している。
観察: 乱数調整ツールでは入力フォームとその結果が主要な操作対象であり、設定パネルは頻繁に操作しない。類似の Web ツール (pokemonrng.com、Chatot 等) ではパラメータ設定をメインコンテンツ上部やインラインに配置するパターンが多く、左サイドバーが常時表示されるレイアウトは少ない。
当面の方針: サイドバーの廃止 (設定をインラインまたはモーダルに統合) か右側配置への変更を含め、FeatureContent の実装 (local_054 以降) で実際の画面構成と合わせて判断する。
